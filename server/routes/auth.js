...
router.post("/login", loginLimiter, async (req, res) => {
  try {
    // Accept both email/username format for flexibility
    ...
    // Find user by email (loginIdentifier should be email)
    ...
    try {
      ...
      if (users.length === 0) {
        ...
        return res.status(404).json({
          message:
            "If an account with this email or phone number exists, we've sent a password reset link.",
        });
      }
